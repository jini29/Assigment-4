...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    T_MUL = 262,                   // T_MUL
    T_DIV = 263,                   // T_DIV
    T_DT_INT = 264,                // T_DT_INT
    T_DT_FLOAT = 265,              // T_DT_FLOAT
    T_LITERAL_STR = 266,           // T_LITERAL_STR
    T_READ = 267,                  // T_READ
    T_WRITE = 268,                 // T_WRITE
    T_SEMICOLON = 269,             // T_SEMICOLON
    T_COMMA = 270,                 // T_COMMA
    T_LS_BRACKET = 271,            // T_LS_BRACKET
    T_RS_BRACKET = 272,            // T_RS_BRACKET
    T_ID = 273,                    // T_ID
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_T_NUM = 3,                             // T_NUM
        S_T_ASSIGN = 4,                          // T_ASSIGN
        S_T_ADD = 5,                             // T_ADD
        S_T_SUB = 6,                             // T_SUB
        S_T_MUL = 7,                             // T_MUL
        S_T_DIV = 8,                             // T_DIV
        S_T_DT_INT = 9,                          // T_DT_INT
        S_T_DT_FLOAT = 10,                       // T_DT_FLOAT
        S_T_LITERAL_STR = 11,                    // T_LITERAL_STR
        S_T_READ = 12,                           // T_READ
        S_T_WRITE = 13,                          // T_WRITE
        S_T_SEMICOLON = 14,                      // T_SEMICOLON
        S_T_COMMA = 15,                          // T_COMMA
        S_T_LS_BRACKET = 16,                     // T_LS_BRACKET
        S_T_RS_BRACKET = 17,                     // T_RS_BRACKET
        S_T_ID = 18,                             // T_ID
        S_T_INTEGER = 19,                        // T_INTEGER
        S_T_FLOAT = 20,                          // T_FLOAT
        S_T_BEGIN = 21,                          // T_BEGIN
        S_T_END = 22,                            // T_END
        S_T_REPEAT = 23,                         // T_REPEAT
        S_T_UNTIL = 24,                          // T_UNTIL
        S_T_DO = 25,                             // T_DO
        S_T_WHILE = 26,                          // T_WHILE
        S_T_IF = 27,                             // T_IF
        S_T_THEN = 28,                           // T_THEN
        S_T_ELSE = 29,                           // T_ELSE
        S_T_LPAR = 30,                           // T_LPAR
        ...
        S_stmt_list = 40,                        // stmt_list
        S_stmt = 41,                             // stmt
        S_block = 42,                            // block
        S_construct_while = 43,                  // construct_while
        S_44_1 = 44,                             // $@1
        S_45_2 = 45,                             // $@2
        S_construct_repeat = 46,                 // construct_repeat
        S_47_3 = 47,                             // $@3
        S_construct_if = 48,                     // construct_if
        S_49_4 = 49,                             // $@4
        S_50_5 = 50,                             // $@5
        S_construct_else = 51,                   // construct_else
        S_52_6 = 52,                             // $@6
        S_l_expr = 53,                           // l_expr
        S_op_rel = 54,                           // op_rel
        S_assignment = 55,                       // assignment
        S_declaration = 56,                      // declaration
        S_array_size = 57,                       // array_size
        S_datatype = 58,                         // datatype
        S_a_expr = 59,                           // a_expr
        S_a_term = 60,                           // a_term
        S_a_fact = 61,                           // a_fact
        S_varref = 62,                           // varref
        S_arr_index = 63,                        // arr_index
        S_read = 64,                             // read
        S_write = 65,                            // write
        S_varlist = 66,                          // varlist
        S_expr_list = 67                         // expr_list
      };
    };
...
switch (yykind)
    {
      ...
      {
        return simple_parser::symbol_name (this->kind ());
      }
...
#endif
      {
        YY_ASSERT (tok == token::T_EOF
                   || (token::YYerror <= tok && tok <= token::T_RS_BRACKET)
                   || (token::T_BEGIN <= tok && tok <= token::T_GEQ)
                   || (40 <= tok && tok <= 41));
      }
#if 201103L <= YY_CPLUSPLUS
    ...
    {
    public:
      context (const simple_parser& yyparser, const symbol_type& yyla);
  ...
  template <typename Base>
  simple_parser::symbol_kind_type
  simple_parser::basic_symbol<Base>::type_get () const YY_NOEXCEPT
  {
    return this->kind ();
  }
  ...
  // by_kind.
  inline
  simple_parser::by_kind::by_kind ()
    : kind_ (symbol_kind::S_YYEMPTY)
  {}

#if 201103L <= YY_CPLUSPLUS
  inline
  simple_parser::by_kind::by_kind (by_kind&& that)
    : kind_ (that.kind_)
  {
    that.clear ();
  }
  ...
  inline
  simple_parser::by_kind::by_kind (const by_kind& that)
    : kind_ (that.kind_)
  {}

  inline
  simple_parser::by_kind::by_kind (token_kind_type t)
    : kind_ (yytranslate_ (t))
  {}

  inline
  void
  simple_parser::by_kind::clear () YY_NOEXCEPT
  {
    kind_ = symbol_kind::S_YYEMPTY;
  }
  ...
  inline
  void
  simple_parser::by_kind::move (by_kind& that)
  {
    kind_ = that.kind_;
    that.clear ();
  }
  ...
  inline
  simple_parser::symbol_kind_type
  simple_parser::by_kind::kind () const YY_NOEXCEPT
  {
    return kind_;
  }
  ...
  inline
  simple_parser::symbol_kind_type
  simple_parser::by_kind::type_get () const YY_NOEXCEPT
  {
    return this->kind ();
  }
